...
const Button = React.forwardRef((props, ref) => {
  ...
  const animationElements = useMemo(
    () => (
      /* The shine effect element can be move out of the scorp and be memorized to avoid unnesaccery re-render */
      <>
        {layout === "primary" && (
          <span
            className="absolute overflow-hidden bg-purple pointer-events-none"
            /* Below the static style can be convert to tailwindcss class */
            style={{
              zIndex: -1,
              top: -1,
              left: -1,
          ...
          >
            <span
              /* Below the static style can be convert to tailwindcss class */
              /* And use classname util to conbine the variable with the tailwindcss */
        ...
        {loadingAnimation && (
          <span
            ...
            /* here the static style can be converte as tailwindcss class */
            /* About the fromLoadingPercent, it already handled by the GASP animation, can be remove. */
            style={{
              zIndex: 1,
          ...
          >
            ...
            {/* Migrate the buttonTextSize into the buttonClasses because them alway combine as one. */}
            <span className={twClassNames(buttonClasses, "bg-purple text-white", buttonTextSize)}>
              {/* The children actually didn't implement and not nesaccery to contain the context, therefore should remove it. */}
              {children}
            </span>
          </span>
        )}
  ...
  const button = (
    <button
      ...
      className={classNames(
        buttonClasses,
        isLoading ? "bg-white text-purple hover:bg-white hover:text-purple active:bg-white active:text-purple" : "",
        buttonTextSize
      )}
      {...forwardProps}
  ...
  const buttonWithMagnet = (
    <span
      className={twClassNames(containerClassName, "-m-4 p-4 rounded-full inline-flex box-content")}
      ref={magnetRef}
  ...
  if (!withoutButtonTag) {
    return (
      <Widget FallbackComponent={ErrorViewTemplateSmall}>{layout === "primary" ? buttonWithMagnet : button}</Widget>
    );
  }
